...
> **Important — if this repo uses the `.ai/` sync harness:** the root agent files (`AGENTS.md`, `CLAUDE.md`, `.github/copilot-instructions.md`, `.agents/context/AGENTS.md`) are **generated** — composed by `.ai/scripts/sync.js` from `.ai/AGENTS.md` (project-specific) + `.ai/_base.md` (portable). Never edit the generated files directly; any "edit the root agent doc" instruction below means *edit the `.ai/` sources and run `node .ai/scripts/sync.js`*. You can tell a file is generated by the `<!-- ai-sync-generated -->` marker at the end. The instructions below apply as written only in a vanilla environment without the sync harness.
